...
`abs_loop` and `abs_set` illustrate these rules. The functions both handle two cases and perform one sequential step, \@ref(fig:abs). If a number is positive, the functions leave it alone. If a number is negative, the functions multiply it by negative one.
...
The plan in \@ref(fig:abs) now requires a sequential step: you must multiply each of the negative values by negative one. All of R's arithmetic operators are vectorized, so you can use `*` to complete this step in vectorized fashion. `*` will multiply each number in `vec[vec < 0]` by negative one at the same time:
...
```{solution}
`change_symbols` uses a `for` loop to sort values into seven different cases, as demonstrated in \@ref(fig:change).
```
...
I refer to these problems as the logistical, tactical, and strategic problems of data science, as shown in \@ref(fig:venn). You'll face them whenever you try to learn from data:
